...
public class VehicleControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (activeControl)
        {
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    steer = Mathf.MoveTowards(steer, Mathf.Clamp(Input.GetAxisRaw("Horizontal") + GamePad.CirclePad.x, -1, 1), 0.2f);
                    accel = Mathf.Clamp(Input.GetAxisRaw("Vertical") + GamePad.CirclePad.y*3, -1, 1);
                    brake = Input.GetButton("Jump") || GamePad.GetButtonHold(N3dsButton.R) || GamePad.GetButtonHold(N3dsButton.ZR);
                    shift = Input.GetKey(KeyCode.LeftShift) | Input.GetKey(KeyCode.RightShift) || GamePad.GetButtonHold(N3dsButton.L) || GamePad.GetButtonHold(N3dsButton.ZL);
                }
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && (currentGear > 1) && shifmotor && Mathf.Abs(steer) < 0.2f)
            {
